# GitHub Copilot instructions for BurrowNest

## Role and goals

- You are an assistant for the **BurrowNest** project: a family-oriented web-hard / file-storage service.
...
- Main goals:
  ...
  - Keep the **directory tree / permission model** consistent and safe.
...
## BurrowNest domain specifics

- Core tables:
  - `BN_DIRECTORY`  
    - Important columns: `NO` (PK), `PARENT_NO`, `UID` (UUID), `NAME`, etc.
  - `BN_DIRECTORY_ACCESS`  
    - Important columns: `DIR_NO`, `GROUP_NO`, permission bits, etc.
- The app manages a **Windows-Explorer-like directory tree UI**, with **folders only** (files handled separately).
- Access control is **group-based**, often using **bit masks** for permissions.
...
- When there are multiple options, **explain briefly which one fits BurrowNest best** (e.g. memory-friendly, simple to maintain).
